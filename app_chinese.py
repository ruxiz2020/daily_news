...
news_type_list = list(all_df_cn_news["type"].unique())
news_source_list = list(all_df_cn_news["news_source"].unique())
...
def description_card():
    ...
    return html.Div(
        ...
        children=[
            #html.H5("每日新闻汇总"),
            #html.H3("Welcome to the Daily News Dashboard"),
            html.Div(
                id="intro",
                children="新闻来源: BBC, WSJ, NYT, Yahoo, google news, baidu news, etc.",
            ),
        ],
    )
...
def generate_control_card():
    ...
    return html.Div(
        id="control-card",
        children=[
            html.P("Select News Type"),
            dcc.Dropdown(
                id="news-type-select",
                options=[{"label": i, "value": i} for i in news_type_list],
                value=news_type_list[0],
            ),
            html.Br(),
            html.P("Select News Source"),
            dcc.Dropdown(
                id="source-select",
                options=[{"label": i, "value": i} for i in news_source_list],
                value=news_source_list[:],
                multi=True,
            ),
            html.Br(),
            #html.Div(
...
app.layout = html.Div(
    ...
    children=[
        ...
        html.Div(
            id="banner",
            className="banner",
            children=[html.Img(src=app.get_asset_url("每日新聞匯總.png"))],
        ),
        # Left column
        html.Div(
            id="left-column",
