...
currency_pair_data = {
    "DJI": pd.read_csv(
        DATA_PATH.joinpath("DJI.csv"), index_col=1, parse_dates=["Date"]
    ),
    "SPX": pd.read_csv(
        DATA_PATH.joinpath("SPX.csv"), index_col=1, parse_dates=["Date"]
    ),
    "SHC": pd.read_csv(
        DATA_PATH.joinpath("SHC.csv"), index_col=1, parse_dates=["Date"]
    ),
    "HSI": pd.read_csv(
        DATA_PATH.joinpath("HSI.csv"), index_col=1, parse_dates=["Date"]
    ),
}

# Currency pairs
currencies = ["DJI", "SPX", "SHC", "HSI"]
            ...
            children=[
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.P(
                            """
                            各大媒体头条新闻,包括: BBC, New York Times, Yahoo, google news, baidu news, etc.
                            Home page 是当天新闻头条汇总。 Chilren pages 是当天汇总新闻关键词的历史搜索还有解析。
                            """
                        ),
                    ],
                ),
